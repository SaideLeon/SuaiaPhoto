...
export const PROMPT_TEMPLATES: PromptTemplate[] = [
  ...
  },
  {
    ...
    prompt: (gender: Gender, customClothing?: string, hasClothingImage?: boolean, hasHairstyleImage?: boolean, _marketingText?: string, aspectRatio: string = '1:1') => {
      ...
      return `Edit the uploaded image to show the same subject as a ${subject} (keeping ${possessivePronoun} ${identityPreservation}) positioned in a close-up portrait shot. ${H_is} face should be tilted slightly upward at approximately 15–20 degrees with ${possessivePronoun} chin gently lifted, creating a confident, aspirational angle. ${H_is} head is centered in the frame, gaze directed straight toward the camera. ${hairstyleInstruction} ${subjectPronoun} is wearing modern, vintage-inspired sunglasses with molten amber-to-gold gradient lenses and lustrous golden frames with fine etched details, positioned perfectly on the bridge of ${possessivePronoun} nose. ${H_is} body is angled slightly (about 30 degrees) to add dimension, with relaxed shoulders and one shoulder subtly closer to the camera. ${H_is} expression should convey calm confidence, with a slight, composed smirk.${clothingInstruction} The background is a rich, saturated golden-yellow that transitions to deeper amber tones at the edges. Dramatic directional lighting from above-left sculpts ${possessivePronoun} face and neck with warm highlights and soft shadows, while subtle backlighting creates a halo effect around ${possessivePronoun} hair. The composition should reflect a fashion/editorial beauty portrait style, with the face occupying about 60% of the frame. Maintain high realism, cinematic lighting, and fine details. Ensure the final image has a ${aspectRatio} aspect ratio.`;
    }
  },
  {
  ...
  },
  {
    ...
    prompt: (_gender: Gender, customClothing?: string, hasClothingImage?: boolean, hasHairstyleImage?: boolean, _marketingText?: string, aspectRatio: string = '1:1') => {
        ...
        return `Transform the uploaded image, maintaining the ${identityPreservation}. Place them in a gritty, neon-drenched Cyberpunk cityscape at night. The background should feature towering futuristic skyscrapers, holographic advertisements, and rain-slicked streets reflecting the vibrant cyan and magenta lights. ${hairstyleInstruction} ${clothingInstruction} The lighting should be dramatic and high-contrast, with strong neon glows casting colored light onto the subject. The atmosphere is cinematic, moody, and futuristic. Ensure photorealistic quality with fine details in the environment and clothing. Ensure the final image has a ${aspectRatio} aspect ratio.`;
    }
  },
  {
    ...
    prompt: (gender: Gender, customClothing?: string, hasClothingImage?: boolean, hasHairstyleImage?: boolean, _marketingText?: string, aspectRatio: string = '1:1') => {
        ...
        return `Recreate the uploaded image as a classic, black and white Vintage Hollywood portrait from the 1940s. Keep the ${identityPreservation}. ${hairstyleInstruction} Use dramatic, high-contrast monochrome lighting, reminiscent of classic film noir cinematography (e.g., Rembrandt or butterfly lighting). ${clothingInstruction} The pose should be poised and confident, looking slightly off-camera. The background should be a simple, dark studio backdrop to focus all attention on the subject. The final image should have a timeless, elegant, and glamorous feel, with a subtle film grain effect to enhance the vintage aesthetic. Ensure ultra-realistic detail and a cinematic quality. Ensure the final image has a ${aspectRatio} aspect ratio.`;
    }
  },
  {
    ...
    prompt: (_gender: Gender, customClothing?: string, hasClothingImage?: boolean, hasHairstyleImage?: boolean, _marketingText?: string, aspectRatio: string = '1:1') => {
        ...
        return `Edit the uploaded image to feature the same subject (keeping ${identityPreservation}) as a space explorer. Place them inside the cockpit of a futuristic spaceship, with a panoramic view of a nebula-filled galaxy through the main viewport. ${clothingInstruction} ${hairstyleInstruction} The interior of the cockpit should be filled with holographic displays and soft, ambient blue lighting reflecting off their suit. The mood is one of wonder, adventure, and advanced technology. Ensure photorealistic quality and cinematic detail. Ensure the final image has a ${aspectRatio} aspect ratio.`;
    }
  },
  {
  ...
  },
  {
    ...
    prompt: (gender: Gender, customClothing?: string, hasClothingImage?: boolean, hasHairstyleImage?: boolean, marketingText?: string, aspectRatio: string = '1:1') => {
      ...
      return `Create a studio fashion editorial poster featuring the uploaded young ${subject}, keeping their ${identityPreservation}. The subject should be sitting confidently on a modern designer chair. ${hairstyleInstruction} ${clothingInstruction} The background should be a pure red with a slight texture. The composition must include oversized, bold white typography across the top that spells ‘${typographyText}’ and some minimal branding text at the bottom. The overall aesthetic must be sharp, vibrant, and reflect a high-fashion design. The final image should be photorealistic with a high-end, polished quality. Ensure the final image has a ${aspectRatio} aspect ratio.`;
    }
  },
  {
