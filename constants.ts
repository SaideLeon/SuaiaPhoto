...
export const PROMPT_TEMPLATES: PromptTemplate[] = [
  {
    ...
    prompt: (gender: Gender, customClothing?: string) => {
  ...
  },
  {
    ...
    prompt: (gender: Gender, customClothing?: string) => {
      ...
      return `Edit the uploaded image to show the same subject as a ${subject} (keeping ${possessivePronoun} real face, hairstyle, skin tone, and identity unchanged) positioned in a close-up portrait shot. ${H_is} face should be tilted slightly upward at approximately 15–20 degrees with ${possessivePronoun} chin gently lifted, creating a confident, aspirational angle. ${H_is} head is centered in the frame, gaze directed straight toward the camera. ${H_is} hair is neatly styled, maintaining natural texture and volume, with subtle light reflections for depth. ${subjectPronoun} is wearing modern, vintage-inspired sunglasses with molten amber-to-gold gradient lenses and lustrous golden frames with fine etched details, positioned perfectly on the bridge of ${possessivePronoun} nose. ${H_is} body is angled slightly (about 30 degrees) to add dimension, with relaxed shoulders and one shoulder subtly closer to the camera. ${H_is} expression should convey calm confidence, with a slight, composed smirk. ${subjectPronoun} wears ${clothing}. The background is a rich, saturated golden-yellow that transitions to deeper amber tones at the edges. Dramatic directional lighting from above-left sculpts ${possessivePronoun} face and neck with warm highlights and soft shadows, while subtle backlighting creates a halo effect around ${possessivePronoun} hair. The composition should reflect a fashion/editorial beauty portrait style, with the face occupying about 60% of the frame. Maintain high realism, cinematic lighting, and fine details.`;
    }
  },
  {
    ...
    prompt: (gender: Gender, customClothing?: string) => {
      ...
      return `Edit the uploaded image to depict the same ${subject} (keeping ${possessivePronoun} real face, expression, hairstyle, skin tone, and identity completely unchanged) in a striking portrait illuminated by dramatic dual-colored lighting. ${H_is} face should be split evenly — one side bathed in a cool blue light, and the other in a vibrant pink/red hue, producing a high-contrast, neon-like visual effect. ${subjectPronoun} should be gazing directly at the viewer with a thoughtful, pensive expression, ${possessivePronoun} left hand resting on ${possessivePronoun} chin. ${subjectPronoun} is wearing ${clothing}. The background must remain dark and indistinct, subtly fading into shadow to emphasize the colorful illumination on ${possessivePronoun} face and upper body. Maintain photorealism, preserving every real facial and anatomical detail of the original subject while only modifying the lighting, color grading, and atmosphere to achieve a cinematic and artistic look.`;
    }
  },
  {
  ...
  },
  {
    ...
    prompt: (gender: Gender, customClothing?: string) => {
      ...
      return `Edit the uploaded image to create a professional, glamorous studio portrait of the same ${subject}, keeping their real face, hairstyle, skin tone, and identity completely unchanged. Style the scene with soft, directional clamshell lighting, producing a subtle chiaroscuro gradient that enhances the facial contours, highlights, and texture of the skin. Dress ${objectPronoun} in ${clothing}, conveying elegance and sophistication. Their pose should be refined and introspective, with their right hand gently resting under their chin, expressing thoughtfulness and confidence. Maintain a solid black background, emphasizing the lighting contrast and cinematic atmosphere. The overall look should reflect high-fashion editorial portraiture, with ultra-realistic detail, premium photographic quality, and a cinematic, modern aesthetic.`;
    }
  },
  {
    title: "Paisagem Urbana Cyberpunk",
    prompt: (_gender: Gender, customClothing?: string) => {
        const clothing = customClothing || 'a stylish dark leather jacket over a tech-wear outfit';
        return `Transform the uploaded image, maintaining the subject's exact real face, hairstyle, and identity. Place them in a gritty, neon-drenched Cyberpunk cityscape at night. The background should feature towering futuristic skyscrapers, holographic advertisements, and rain-slicked streets reflecting the vibrant cyan and magenta lights. Dress the subject in ${clothing}. The lighting should be dramatic and high-contrast, with strong neon glows casting colored light onto the subject. The atmosphere is cinematic, moody, and futuristic. Ensure photorealistic quality with fine details in the environment and clothing.`;
    }
  },
  {
    title: "Floresta de Conto de Fadas",
    prompt: (_gender: Gender, customClothing?: string) => {
        const clothing = customClothing || "rustic, fantasy-inspired attire, such as a traveler's cloak or simple elven-style clothing";
        return `Edit the uploaded image to place the same subject (keeping their real face, skin tone, and identity unchanged) within an enchanted, fairy tale forest. The environment should be filled with ancient, moss-covered trees, glowing ethereal flora, and soft sunlight filtering through the dense canopy, creating visible god rays. Dress the subject in ${clothing}. The lighting should be magical and soft, with a warm, golden glow. The mood is whimsical, serene, and mystical. Maintain high realism, blending the subject seamlessly into the magical environment.`;
    }
  },
  {
    ...
    prompt: (gender: Gender, customClothing?: string) => {
        ...
        const clothing = customClothing || defaultClothing;
        return `Recreate the uploaded image as a classic, black and white Vintage Hollywood portrait from the 1940s. Keep the subject's real face, expression, and identity completely unchanged. Use dramatic, high-contrast monochrome lighting, reminiscent of classic film noir cinematography (e.g., Rembrandt or butterfly lighting). Dress the subject in ${clothing}. The pose should be poised and confident, looking slightly off-camera. The background should be a simple, dark studio backdrop to focus all attention on the subject. The final image should have a timeless, elegant, and glamorous feel, with a subtle film grain effect to enhance the vintage aesthetic. Ensure ultra-realistic detail and a cinematic quality.`;
    }
  }
];
