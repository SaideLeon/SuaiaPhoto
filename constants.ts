...
export const PROMPT_TEMPLATES: PromptTemplate[] = [
  {
    ...
    prompt: (gender: Gender, customClothing?: string, hasClothingImage?: boolean, hasHairstyleImage?: boolean, _marketingText?: string) => {
  ...
  },
  {
    ...
    prompt: (_gender: Gender, customClothing?: string, hasClothingImage?: boolean, hasHairstyleImage?: boolean, _marketingText?: string) => {
        ...
        return `Transform the uploaded image, maintaining the ${identityPreservation}. Place them in a gritty, neon-drenched Cyberpunk cityscape at night. The background should feature towering futuristic skyscrapers, holographic advertisements, and rain-slicked streets reflecting the vibrant cyan and magenta lights. ${hairstyleInstruction} ${clothingInstruction} The lighting should be dramatic and high-contrast, with strong neon glows casting colored light onto the subject. The atmosphere is cinematic, moody, and futuristic. Ensure photorealistic quality with fine details in the environment and clothing.`;
    }
  },
  {
    ...
    prompt: (gender: Gender, customClothing?: string, hasClothingImage?: boolean, hasHairstyleImage?: boolean, _marketingText?: string) => {
        ...
        return `Recreate the uploaded image as a classic, black and white Vintage Hollywood portrait from the 1940s. Keep the ${identityPreservation}. ${hairstyleInstruction} Use dramatic, high-contrast monochrome lighting, reminiscent of classic film noir cinematography (e.g., Rembrandt or butterfly lighting). ${clothingInstruction} The pose should be poised and confident, looking slightly off-camera. The background should be a simple, dark studio backdrop to focus all attention on the subject. The final image should have a timeless, elegant, and glamorous feel, with a subtle film grain effect to enhance the vintage aesthetic. Ensure ultra-realistic detail and a cinematic quality.`;
    }
  },
  {
  ...
  },
  {
    ...
    prompt: (gender: Gender, customClothing?: string, hasClothingImage?: boolean, hasHairstyleImage?: boolean, marketingText?: string) => {
      const subject = gender === 'masculino' ? 'man' : 'woman';
